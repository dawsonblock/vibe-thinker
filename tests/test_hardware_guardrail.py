...
class TestEstimateModelRam:
    def test_local_file_returns_estimate(self, tmp_path):
        # Create a fake 100MB model file.
        f = tmp_path / "model.gguf"
        f.write_bytes(b"\x00" * (100 * 1024 * 1024))
        ram = _estimate_model_ram_mb(str(f), n_ctx=4096, pool_size=1)
    ...
    def test_pool_size_scales_ram(self, tmp_path):
        f = tmp_path / "model.gguf"
        f.write_bytes(b"\x00" * (100 * 1024 * 1024))
        ram1 = _estimate_model_ram_mb(str(f), n_ctx=4096, pool_size=1)
        ram4 = _estimate_model_ram_mb(str(f), n_ctx=4096, pool_size=4)
        assert ram4 > ram1
        # 4 instances should cost roughly 4x (minus the shared KV cache
        # overhead, but with the safety multiplier it should be close).
        assert ram4 >= ram1 * 3

    def test_n_ctx_affects_kv_cache(self, tmp_path):
        f = tmp_path / "model.gguf"
        f.write_bytes(b"\x00" * (100 * 1024 * 1024))
        ram_small = _estimate_model_ram_mb(str(f), n_ctx=512, pool_size=1)
        ram_large = _estimate_model_ram_mb(str(f), n_ctx=8192, pool_size=1)
...
class TestCheckModelFitsRam:
    ...
    def test_large_model_does_not_fit(self, tmp_path):
        f = tmp_path / "huge.gguf"
        f.write_bytes(b"\x00" * (5000 * 1024 * 1024))  # 5GB
        result = check_model_fits_ram(
            str(f), n_ctx=4096, pool_size=1, available_ram_mb=2048,
        )
    ...
    def test_error_includes_remediation(self, tmp_path):
        f = tmp_path / "big.gguf"
        f.write_bytes(b"\x00" * (3000 * 1024 * 1024))  # 3GB
        result = check_model_fits_ram(
            str(f), n_ctx=4096, pool_size=2, available_ram_mb=1024,
        )
        assert result.ok is False
    ...
    def test_pool_size_in_error_message(self, tmp_path):
        f = tmp_path / "big.gguf"
        f.write_bytes(b"\x00" * (2000 * 1024 * 1024))  # 2GB
        result = check_model_fits_ram(
            str(f), n_ctx=4096, pool_size=4, available_ram_mb=1024,
        )
        assert result.ok is False
