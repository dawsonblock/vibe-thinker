...
class TestReliabilityScoring:
    ...
    def test_no_answer_returns_zero(self, clr):
        # Even with all-verified claims, no claims means score 0
        assert clr._calculate_reliability([1, 1, 1], claims=["a" * 20, "b" * 20, "c" * 20]) == 1.0

    def test_fewer_than_min_claims_returns_zero(self, clr):
        # Only 2 meaningful claims — below MIN_CLAIMS_FOR_SCORING=3
        assert clr._calculate_reliability([1, 1], claims=["a" * 20, "b" * 20]) == 0.0

    def test_single_claim_returns_zero(self, clr):
        # The smoking gun from the audit: 1 verified claim -> 1.0
        # Now it must return 0.0
        assert clr._calculate_reliability([1], claims=["a meaningful claim here"]) == 0.0
    ...
    def test_garbage_claims_rejected(self, clr):
        ...
        assert clr._calculate_reliability([1, 1, 1], claims=claims) == 0.0
    ...
    def test_any_failed_verdict_capped(self, clr):
        ...
        score = clr._calculate_reliability([1, 1, 1, 1, 0], claims=claims)
        assert score <= 0.3
        assert score > 0.0  # not zero, but heavily penalized

    def test_all_verified_meaningful_claims_high_score(self, clr):
        claims = ["a" * 20, "b" * 20, "c" * 20, "d" * 20, "e" * 20]
        score = clr._calculate_reliability([1, 1, 1, 1, 1], claims=claims)
        assert score == 1.0

    def test_mixed_garbage_and_real_claims(self, clr):
        # 2 garbage + 3 real, all verified -> only 3 count, score 1.0
        claims = ["by step.", "short", "real claim one here", "real claim two here", "real claim three here"]
        score = clr._calculate_reliability([1, 1, 1, 1, 1], claims=claims)
        assert score == 1.0
