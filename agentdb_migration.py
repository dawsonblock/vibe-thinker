"""AgentDB migration helpers: backfill + recall verification.

This module is the importable source of truth for the migration logic.
The standalone CLI wrapper lives at ``scripts/migrate_to_agentdb.py`` and
``rfsn_cli.py finalize-migration`` imports from here so the commands work
...
def verify_recall(
    ...
    ``sample_size`` entries, runs a search query using each entry's own
    embedding against both the local store (in-memory) and AgentDB, and
    compares the top_k results. Recall is the fraction of queries where
    AgentDB's top_k result set overlaps with the local store's top_k result
    set by at least 50% (i.e. at least ceil(top_k/2) of the local results
    appear in AgentDB's results).
    ...
    for store_name, path, id_prefix in [
        ...
        sample_indices = list(range(
            0, len(entries), max(1, len(entries) // sample_size)
        ))[:sample_size]
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Backfill local cache entries into AgentDB for "
                    "migration. Read-only on local files."
    )
    p.add_argument(
        "--agentdb-url",
        required=True,
...
def check_agentdb_reachable(agentdb: AgentDBVectorStore) -> bool:
    ...
    and upsert() silently fails (prints a warning, doesn't raise). So we verify
    by: upsert a test entry, search for it, and check it was actually stored.
    If the search returns nothing, AgentDB is unreachable.
    """
...
def main() -> int:
    ...
    if not args.dry_run:
        reachable = check_agentdb_reachable(agentdb)
        if not reachable:
    ...
    if vres["passed"]:
        print("[Verify] PASSED -- AgentDB recall meets threshold. "
              "You can now run `vibe-thinker finalize-migration` "
